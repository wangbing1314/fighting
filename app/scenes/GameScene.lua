...
GameWin = require("app.scenes.GameWin")
GameOver = require("app.scenes.GameOver")
...
function GameScene:timeDeal( ... )
    ...
    self:removeChild(sp)
    sp = nil
    local distance = math.sqrt((display.cx - hPosX) * (display.cx - hPosX) + (display.cy + 120 - hPosY) * (display.cy + 120 - hPosY))
    sp = CCSprite:createWithTexture(a, CCRectMake(0, 0, 2, distance))
    sp:setRotation(self._hook:getRotation())
    sp:setAnchorPoint(ccp(0.5, 1))
    sp:setPosition(ccp(display.cx, display.cy + 120))
    self:addChild(sp)
    for k,v in pairs(self.goods_tab) do
